...
export function useCryptoMarket(initialCryptoList = []) {
  const cryptoList = ref(initialCryptoList)
  ...
  const fetchMarketData = async () => {
    ...
    try {
      ...
      // 2. 获取 K 线数据（并行）
      const klinePromises = cryptoList.value.map(async (crypto) => {
        try {
          ...
          console.warn(`获取 ${crypto.symbol} K线数据失败:`, err.message)
        }
      })
  ...
  /**
   * 开始自动更新
   * @param {number} interval - 更新间隔（毫秒），默认5分钟
   */
  const startAutoUpdate = (interval = 300000) => {
    fetchMarketData()
    timer = setInterval(fetchMarketData, interval)
  }
