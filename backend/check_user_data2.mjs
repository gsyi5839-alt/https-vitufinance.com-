...
async function check() {
  ...
  // 4. 检查 transaction_history 表是否有相关记录
  console.log('\n=== Transaction History (exact match) ===');
  const [txHistory] = await conn.execute('SELECT * FROM transaction_history WHERE LOWER(wallet_address) = ?', [targetAddress.toLowerCase()]);
