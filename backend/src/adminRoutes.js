...
router.get('/team-management/search', authMiddleware, async (req, res) => {
  try {
    ...
    if (min_level > 0) {
      whereClause += ' AND COALESCE(bl.level, 0) >= ?';
    ...
    // Get users with team stats
    const users = await dbQuery(`
      SELECT 
        ub.wallet_address, ub.usdt_balance, ub.total_deposit, ub.created_at,
        COALESCE(bl.level, 0) as broker_level,
        (SELECT COUNT(*) FROM user_referrals WHERE referrer_address = ub.wallet_address) as direct_referrals,
        (SELECT COALESCE(SUM(reward_amount), 0) FROM referral_rewards WHERE wallet_address = ub.wallet_address) as total_referral_reward,
        (SELECT COALESCE(SUM(reward_amount), 0) FROM team_rewards WHERE wallet_address = ub.wallet_address) as total_team_dividend
      FROM user_balances ub
      LEFT JOIN broker_levels bl ON bl.wallet_address = ub.wallet_address
      WHERE ${whereClause}
      ORDER BY COALESCE(bl.level, 0) DESC, ub.total_deposit DESC
      LIMIT ? OFFSET ?
    `, [...params, parseInt(limit), offset]);
    ...
    console.error('[TeamManagement] Search error:', error);
    res.status(500).json({ success: false, message: 'Failed to search users' });
  }
});

/**
 * GET /api/admin/team-management/user/:wallet_address
 * Get user complete team data
 */
router.get('/team-management/user/:wallet_address', authMiddleware, async (req, res) => {
  try {
    ...
    const userRows = await dbQuery(`
      ...
      FROM user_balances ub WHERE ub.wallet_address = ?
    `, [walletAddr]);
    ...
    // Get team stats using recursive CTE
    const teamStats = await dbQuery(`
      WITH RECURSIVE team_tree AS (
        SELECT wallet_address, 1 as level FROM user_referrals WHERE referrer_address = ?
        UNION ALL
        SELECT r.wallet_address, t.level + 1 FROM user_referrals r
        INNER JOIN team_tree t ON r.referrer_address = t.wallet_address WHERE t.level < 8
      )
      SELECT COUNT(DISTINCT wallet_address) as total_team_members,
    ...
    const teamDividends = await dbQuery(`SELECT COALESCE(SUM(reward_amount), 0) as total FROM team_rewards WHERE wallet_address = ?`, [walletAddr]);
    const brokerInfo = await dbQuery(`SELECT level as broker_level, direct_count as qualified_direct_members, team_volume as team_performance FROM broker_levels WHERE wallet_address = ?`, [walletAddr]);
    ...
    console.error('[TeamManagement] Get user error:', error);
  ...
  try {
    ...
    const buildTree = async (address, level) => {
      ...
      const children = await dbQuery(`
        SELECT r.wallet_address, r.created_at as bind_time,
          COALESCE(ub.usdt_balance, 0) as balance, COALESCE(ub.total_deposit, 0) as total_deposit,
        ...
        WHERE r.referrer_address = ? ORDER BY r.created_at DESC LIMIT 100
      `, [address]);
    ...
    const countNodes = (nodes) => {
      ...
      for (const node of nodes) { if (node.children) count += countNodes(node.children); }
      return count;
    };
    ...
    res.json({
      success: true,
      data: {
        root: walletAddr, max_level: maxLvl, total_members: countNodes(hierarchy),
        direct_members: hierarchy.length, hierarchy: hierarchy
      }
    });
  } catch (error) {
    console.error('[TeamManagement] Hierarchy error:', error);
...
router.post('/team-management/award-referral', authMiddleware, async (req, res) => {
  try {
    ...
    await dbQuery(`INSERT INTO referral_rewards (wallet_address, from_wallet, reward_amount, level, source_type, source_id, created_at)
      VALUES (?, 'system', ?, 0, 'admin_award', 0, NOW())`, [walletAddr, awardAmount]);

    // Record transaction
    await dbQuery(`INSERT INTO transaction_history (wallet_address, tx_type, amount, token, description, status, created_at)
    ...
    // Log operation
    await dbQuery(`INSERT INTO admin_operation_logs (admin_id, admin_username, operation_type, operation_target, operation_detail, ip_address, created_at)
    ...
    res.json({ success: true, message: `Successfully awarded ${awardAmount} USDT referral bonus`, data: { wallet_address: walletAddr, amount: awardAmount }});
  } catch (error) {
    console.error('[TeamManagement] Award referral error:', error);
...
router.post('/team-management/award-dividend', authMiddleware, async (req, res) => {
  try {
    ...
    await dbQuery(`INSERT INTO team_rewards (wallet_address, broker_level, reward_amount, reward_date, created_at)
      VALUES (?, ?, ?, ?, NOW())`, [walletAddr, level, awardAmount, today]);
    ...
    res.json({ success: true, message: `Successfully awarded ${awardAmount} USDT team dividend`, data: { wallet_address: walletAddr, amount: awardAmount, broker_level: level }});
  } catch (error) {
    console.error('[TeamManagement] Award dividend error:', error);
...
router.put('/team-management/broker-level', authMiddleware, async (req, res) => {
  try {
    ...
    // Get current level
    const currentRows = await dbQuery('SELECT level as broker_level FROM broker_levels WHERE wallet_address = ?', [walletAddr]);
    const oldLevel = currentRows[0]?.broker_level || 0;

    // Update or insert broker level
    await dbQuery(`INSERT INTO broker_levels (wallet_address, level, updated_at) VALUES (?, ?, NOW())
      ON DUPLICATE KEY UPDATE level = ?, updated_at = NOW()`, [walletAddr, brokerLevel, brokerLevel]);

    // Log operation
    await dbQuery(`INSERT INTO admin_operation_logs (admin_id, admin_username, operation_type, operation_target, operation_detail, ip_address, created_at)
      VALUES (?, ?, 'CHANGE_BROKER_LEVEL', ?, ?, ?, NOW())`,
      [req.admin?.id || 0, adminUsername, walletAddr, JSON.stringify({ old_level: oldLevel, new_level: brokerLevel, reason }), req.ip]);

    secureLog('info', `Broker level changed: ${walletAddr} from ${oldLevel} to ${brokerLevel} by ${adminUsername}`);

    res.json({ success: true, message: `Broker level updated from ${oldLevel} to ${brokerLevel}`, data: { wallet_address: walletAddr, old_level: oldLevel, new_level: brokerLevel }});
  } catch (error) {
    console.error('[TeamManagement] Level change error:', error);
  ...
  try {
    ...
    for (const award of awards) {
      try {
        ...
        if (!wallet_address || !amount || amount <= 0) { failed++; results.push({ wallet_address, status: 'invalid' }); continue; }
        ...
        // Check user exists
        const userCheck = await dbQuery('SELECT wallet_address FROM user_balances WHERE wallet_address = ?', [walletAddr]);
        if (!userCheck || userCheck.length === 0) { failed++; results.push({ wallet_address: walletAddr, status: 'not_found' }); continue; }
        ...
        if (award_type === 'referral') {
          await dbQuery(`INSERT INTO referral_rewards (wallet_address, from_wallet, reward_amount, level, source_type, source_id, created_at)
            VALUES (?, 'system', ?, 0, 'batch_admin_award', 0, NOW())`, [walletAddr, awardAmount]);
        } else {
          const today = new Date().toISOString().slice(0, 10);
          await dbQuery(`INSERT INTO team_rewards (wallet_address, broker_level, reward_amount, reward_date, created_at)
            VALUES (?, ?, ?, ?, NOW())`, [walletAddr, broker_level, awardAmount, today]);
        }

        // Record transaction
        await dbQuery(`INSERT INTO transaction_history (wallet_address, tx_type, amount, token, description, status, created_at)
          VALUES (?, ?, ?, 'USDT', ?, 'completed', NOW())`,
          [walletAddr, `batch_${award_type}_award`, awardAmount, reason || `Batch award by ${adminUsername}`]);

        success++; totalAmount += awardAmount;
        results.push({ wallet_address: walletAddr, amount: awardAmount, status: 'success' });
      } catch (err) { failed++; results.push({ wallet_address: award.wallet_address, status: 'error', error: err.message }); }
    }

    // Log batch operation
    await dbQuery(`INSERT INTO admin_operation_logs (admin_id, admin_username, operation_type, operation_target, operation_detail, ip_address, created_at)
      VALUES (?, ?, 'BATCH_AWARD', 'batch', ?, ?, NOW())`,
      [req.admin?.id || 0, adminUsername, JSON.stringify({ award_type, count: success, total_amount: totalAmount, reason }), req.ip]);

    secureLog('info', `Batch ${award_type} award: ${success} users, ${totalAmount} USDT by ${adminUsername}`);

    res.json({ success: true, message: `Awarded ${success} users, failed ${failed}`, data: { success, failed, total_amount: totalAmount, results: results.slice(0, 50) }});
  } catch (error) {
    console.error('[TeamManagement] Batch award error:', error);
