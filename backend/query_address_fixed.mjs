...
async function runQueries() {
  ...
  try {
    ...
    const [deposits] = await connection.query(
      'SELECT id, wallet_address, amount, token, network, tx_hash, status, created_at FROM deposit_records WHERE LOWER(wallet_address) = ? ORDER BY created_at DESC',
    ...
    const [recent] = await connection.query(
      'SELECT id, wallet_address, amount, token, network, status, created_at FROM deposit_records ORDER BY id DESC LIMIT 10'
    );
